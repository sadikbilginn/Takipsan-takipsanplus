...
class Package{
    ...
    constructor(
        ...
        this.itemsCount     = itemsCount ? itemsCount : '' ;
...
class Consignment{
            ...
            if(!this.epcs.has(tag.epc)){
                ...
                if (this.isLastBoxClosed){
                    ...
                    if(auto_size_name == 1 &&
                        ...
                        last_size = $('#consignmentList tbody tr:first-child td:nth-child(4)').text();
                        //alert(last_size);
                    }
                }
    ...
    addPackage = function (tag){
        ...
        if (!this.packages.has(tag.packageNo)){
            ...
            this.packages.set(
                tag.packageNo,
                //this.packages.set(tag.packageNo, new Package(tag.packageNo, tag.itemsCount,  null, tag.items,tag.box_type_id,tag.load_type,tag.barcode,tag.colour,tag.singles));
                new Package(
                    tag.packageNo,
                    tag.itemsCount,
                    null,
            ...
            //console.log(tag.barcode +'_barkod bastı');
    ...
    checkClose = function(boxCloseTime){
        ...
        if(!this.isLastBoxClosed){
            ...
            var currentPackage = this.packages.get(this.packageNo);
            var hangingProduct = localStorage.getItem('hangingProduct');
            ...
            if (hangingProduct == 1){
                ...
                if (boxCloseTime == 0 ){
                    ...
                    var packageId = localStorage.getItem('activePackageId');
                    ...
                    axios({
                        ...
                        // kayıt paket listesi yenilenicek. getProductMsDetails ile veri gonder.
                        modalClose();
                        stopReader(readerId);
                        ...
                        //updateID(packageId, response.data);
                        //console.log(response.data);
                        console.log('save');

                        MSdatatable.ajax.reload();

                        $('#totalQuantity').html(response.data+'<br> ftm');
        ...
        $('input[type=checkbox].check:checked').each(function () {
            ...
            //if(size == "" || size == null || size == "-"){
                ...
                packages.get(packageNo).items.forEach((values, keys)=>{
                    new_size = "UND";
                    //alert(values.gtin.length);
                    var gtinEpcUzunluk = values.gtin.substring(values.gtin.length - 8);
                    //alert(gtinEpcUzunluk);
                    if (gtinEpcUzunluk.substr(0, 1) == 0){
                        gtinEpcUzunluk = values.gtin.substring(values.gtin.length - 7);
                    }
...
function updateSizes(sizeMap){
    $('#consignmentDetails tbody tr').remove();
    //var packageId = localStorage.getItem('activePackageId');
    //$('#size_'+packageId).text("");
    sizeMap.forEach(function (size, key) {
        if(size != '' && size != 0){
            $('#consignmentDetails tbody').append(
            '<tr>'
                +'<td>'+key+'</td><td>'+size+'</td>'
            +'</tr>'
            );

            //$('#size_'+packageId).append(key +',');
...
async function insertFromDbPackage(data){
    ...
    await  $.each(data.reverse(), function(i, item) {
        ...
        package.colourCode      = item.colourCode;
        package.colourDesc      = item.colourDesc;
        package.singles         = item.singles;
        package.upc             = item.upc;
        package.departmentDesc  = item.departmentDesc;
        package.strokeDesc      = item.strokeDesc;
