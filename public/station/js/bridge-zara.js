...
$(function () {
    ...
    if(consignmentId !== null){
        ...
        //$('#consignments').trigger('change.select2');
        // $('#consignments').val(null).trigger('change');
        // $('#consignments').val(consignmentId).trigger('change');
...
function updateItemLeft(value){
    $('#itemLeft').text( $("#itemCount").text() - value );
}
...
async function insertFromDbPackage(data){
    ...
    await  $.each(data.reverse(), function(i, item) {
        ...
        tag.itemsCount      = item.items.length;
