class Bridge extends WebSocket {
    ...
    onMessageHandler = function(event){
        ...
        try{
            ...
            } else if(msg.type == "barcodeReturn"){
                //console.log("tag reported:");
                //console.log(this);
                //alert(msg.body);
				barcodeReturn(msg.body);
            }
            // gpio responses
...
class Reader {
    ...
    connect = function (connectCallback) {
        ...
        request.body.param = {};
        request.body.param = this.settings;
        request.body.param.isWeighing = false;
        request.body.param.weighing = '';
        request.body.param.isBarcode = barcode_status == "Açık" ? true:false;
        request.body.param.BarcodeIp = barcode_ip_address;
        request.body.param.bridgeCloseTime = bridgeCloseTime;
        //request.body.param.BarcodePort = barcode_port;
        //console.log(">>>>>>>>>>>>>>>>>>>>>");
        //console.log(request);
        this.socket.sendRequest(request);
        //console.log("connecting5");
...
function disconnectReader(id){
    ...
    if (id != 0 ) {
        ...
        br.readers[id].disconnectPromise().then(function (result) {
            connectReaderStatus = false;
            $("#startStop").prop("disabled", true);
            $("#findPackage").prop("disabled", true);
            return true;
