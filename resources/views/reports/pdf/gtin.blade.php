...
<div class="content">
    ...
    <h3 style="text-align: center;">@lang('portal.gtin_list')</h3>
    @if(count($consignment->packages) > 0)
        @foreach($consignment->packages as $key => $value)
            <table cellspacing="0" cellpadding="0" width="100%" style="margin-bottom: 20px;">
                <thead>
                    <tr>
                        <th colspan="4">@lang('portal.package') {{ $value->package_no }} (@lang('portal.total') : {{ count($value->items)  }})</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        @foreach($value->items as $key2 => $val)
                            <td style="font-size: 8px;">{{ $val->gtin }}</td>
                            @if(($key2 + 1) % 4 == 0)
                    </tr>
                    <tr>
                        @endif
