...
@section('content')
    ...
    <div class="animated fadeIn test" style="position:relative; z-index:99;">
        ...
        @include('station.read.read_target')
...
@section('js')
    ...
    <script src="/station/js/bridge-target.js" crossorigin="anonymous"></script>
    <script type="text/javascript">
        ...
        $(function() {
            ...
            //alert(getGTINFromEPC('30340BFC344F5DCBA43B7D21'));
        ...
        $("#consignments").on('change', function () {
            ...
            getProductTargetDetails(consignmentId,selected);

        });

        //Detay Verileri
        function getProductTargetDetails(consignmentId,selected){
            axios({
                url   : stationAjaxUrl,
                method: 'post',
                data  : {
                    process : 'getProductTargetDetails',
        ...
        function insertRow(packageNo,package){
            ...
            if(package && package.items){
                ...
                //alert(gtin);
                var prds =  prodDetails.filter(x => x.item_barcode == gtin);
                ...
                var prod = prds && prds.length > 0 ? prds[0] : null;
                    ...
                    package.items.forEach((values, keys)=>{
                        ...
                        prds =  prodDetails.filter(x => x.item_barcode == values.gtin);
                    ...
                    if( package.items)
                    {
                        if(prod)
                        {
                            ...
                            if (prod.item_description.indexOf(',' != -1)) {
                                prod.item_description = prod.item_description.split(',')[0];
                            }
                        }else{
                ...
                if(packageRow.length > 0){
                    ...
                    '<td id="description_' + package.id + '">' + (prod ? prod.item_description + ' - ' + prd.color : 'UND') + '</td>' +
