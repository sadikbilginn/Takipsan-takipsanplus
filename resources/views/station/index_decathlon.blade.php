...
@section('content')
    ...
    <div class="animated fadeIn test" style="position:relative; z-index:99;">
        <div class="row header">
            ...
            <div class="col-lg-3">
                <div class="row consignment-info">
                    <div class="col-12">Firma: <span id="conn" ></span></div>
        ...
        @include('station.read.read_decathlon')
...
@section('js')
    ...
    <script src="/station/js/bridge.js" crossorigin="anonymous"></script>
    <script type="text/javascript">
        ...
        function insertRow(packageNo,package){
            ...
            if(package && package.items){
                ...
                if(packageRow.length > 0){
                    ...
                    '    <input type="checkbox" class="custom-control-input check" onchange="checkClick(this);" checked package-no="' + pNo + '" package-id="' + pNo + '" id="customCheck'+ pNo + '" data-id="' + package.id + '" value="'+ package.id +'">' +
                    '    <label class="custom-control-label" for="customCheck'+ pNo + '">&nbsp;</label>' +
                    '   </div>' +
                    '</td>' +
                    '<td>@lang('station.package') '+ pNo +'</td>' +
                    ...
                    '<td id="count_' + package.id + '">' + package.itemsCount + '</td>' +
                    '<td id="size_' + package.id + '">' +  package.size  + '</td>' +
                    '</tr>';
        ...
        function deletePackage() {
            ...
            var allVals = [];
            $(".check:checked").each(function() {
                allVals.push($(this).attr('package-id') + '_' + $(this).attr('gtin') );
            });
                ...
                if (result.value) {
                    ...
                    axios({
                        ...
                        data  : {
                            ...
                            consignmentId   : $('#consignments').val(),
                            CloseShipmentSt : CloseShipmentValue
                        }
                    }).then(function (response) {
        ...
        function typesEdit(e,package) {
            ...
            if(boxTypes && boxTypes.length > 0)
            {
                for (let i = 0; i < boxTypes.length; i++) {
                    if(boxTypes[i].name != currentPType){
                        options += "<option value='" + boxTypes[i].name + "'>" + boxTypes[i].name + '   ' + boxTypes[i].length + 'x' + boxTypes[i].width + 'x' + boxTypes[i].height + "</option>";
                    }else{
                        options += "<option selected value='" + boxTypes[i].name + "'>" + boxTypes[i].name + '   ' + boxTypes[i].length + 'x' + boxTypes[i].width + 'x' + boxTypes[i].height + "</option>";
                    }
                }
            }
                ...
                html:'<div class="form-row">' +
                            '<div class="form-group col-md-6">' +
                                ...
                                '<select id="load_type" class="form-control">' +
                                    ...
                                    '<option ' + (currentLType == 'Assortment' ? 'selected' : '') + '  value="Assortment">Assortment</option>' +
                                    '<option ' + (currentLType == 'AssortmentLast' ? 'selected' : '') + '  value="AssortmentLast">Assortment Last</option>' +
                                    '<option ' + (currentLType == 'Solid' ? 'selected' : '') + ' value="Solid">Solid</option>' +
                                    '<option ' + (currentLType == 'SolidLast' ? 'selected' : '') + '  value="SolidLast">Solid Last</option>' +
                                    '<option ' + (currentLType == 'SolidMix' ? 'selected' : '') + '  value="SolidMix">Solid Mix</option>' +
                                '</select>' +
                            '</div>' +
