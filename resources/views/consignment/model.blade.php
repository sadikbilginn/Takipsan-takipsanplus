            ...
            <tr>
                @foreach($consignment->packages as $key => $value)
                    <td style="border: 1px solid #000;">
                        <b style="display:block;padding-bottom: 5px;font-size: 12px;"> @lang('portal.package') {{ $value->package_no }} </b>
                        <table cellspacing="0" cellpadding="0" width="100%">
                            <tr>
                                <td style="text-align:center;border: 1px solid #000;" colspan="2">
                                    <b>{{ $value->model && $value->model != '-' && $value->model != '' ? $value->model : 'UND' }}</b>
                                </td>
                            </tr>
                            @if($value->size && $value->size != '-' && $value->size != '')
                                ...
                                <tr>
                                    <td style='text-align:center;border: 1px solid #000'> {{ $value->size }}</td>
                                    <td style='text-align:center;border: 1px solid #000'> {{ count($value->items) }} @lang('portal.piece')</td>
                                </tr>
                            @else
                                @foreach(getEpcListSize($value->items) as $key2 => $value2)
                                    ...
                                    @endif
                                    <tr>
                                        <td style='text-align:center;border: 1px solid #000'> {{ $key2 }}</td>
                                        <td style='text-align:center;border: 1px solid #000'> {{ $value2 }}  @lang('portal.piece')</td>
                                    </tr>
                                @endforeach
                            @endif
                        </table>
                        <b style="display:block;padding-top: 5px;">@lang('portal.total_product'):{{ count($value->items) }}</b>
                    </td>
                    @if(($key + 1) %4 == 0)
            </tr>
            <tr>
    ...
    @if(isset($sizes) && count($sizes) > 0)
        ...
        <table cellspacing="0" cellpadding="0" width="100%" style="margin-bottom: 20px;">
            ...
            @foreach($sizes as $key => $value)
                <tr>
                    <th width="100">AAAAAAAAAA {{ $key }}</th>
