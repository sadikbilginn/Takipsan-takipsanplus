...
* added $xls->activeSheet (thx reptile)

## 0.8.30 (2021-12-01)

* added $limit to $xls->rows( $sheetIndex = 0, $limit = 0 ) and to $xls->rowsEx( $seetIndex = 0, $limit = 0)
...
* x10 fastest getCell, thx [Jonowa](https://github.com/shuchkin/simplexls/issues/102)
...
* libxml_disable_entity_loader and PHP 8, thx [iKlsR](https://github.com/shuchkin/simplexls/issues/96)
...
* fixed LibreOffice xml specificity, thx [stephengmatthews](https://github.com/shuchkin/simplexls/issues/77)
...
* removed list( $x, $y ), added bool $xls->skipEmptyRows, $xls->parseFile( $filename ), $xls->parseData( $data ), release 0.8.8
...
* simplexls::ParseErrno(), $xls->errno() returns error code
...
* rename simplexls.php to simplexls.php, rename parse_error to parseError fix _columnIndex, add ->toHTML(), GNU to MIT license
...
* $xls->datetime_format to force dates out
